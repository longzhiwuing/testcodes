package com.lzwing.testcode.thread.volatiletest;
...
/**
 * volatiletest 关键字主要作用就是使变量在多个线程中可见。
 * volatiletest 关键字不具备原子性，但Atomic类是具备原子性和可见性。
...
//      测试volatile 关键字的可见性
        ...
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                itDragonVolatile.volatileMethod();
            }
        });
        thread.start();
        ...
        for (int i = 0; i < 100; i++) {
            threads.add(new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    //System.out.println(itDragonVolatile.atomicCountMethod());
                    System.out.println(itDragonVolatile.volatileCountMethod());
                }
            }));
        }
